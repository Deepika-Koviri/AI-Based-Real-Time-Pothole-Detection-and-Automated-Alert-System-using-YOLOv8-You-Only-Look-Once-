...
# from app import app  # ADD THIS LINE (access socketio)
from datetime import datetime
...
def dashboard():
    if request.method == "POST":
        ...
        img_id = current_app.extensions['pymongo'].db.images.insert_one(img_doc).inserted_id
        ...
        for det in detections:
            ...
            # ).inserted_id
            pothole_id = current_app.extensions['pymongo'].db.potholes.insert_one({"current_version": None}).inserted_id
            ...
            v_id = current_app.extensions['pymongo'].db.pothole_versions.insert_one(vdoc).inserted_id
            ...
            current_app.extensions['pymongo'].db.potholes.update_one({"_id": pothole_id}, {"$set": {"current_version": v_id}})
                # ===== ADD THIS BLOCK AFTER THE FOR LOOP ENDS =====
    # socketio = app.config['SOCKETIO']
            # SocketIO - CORRECT (8 spaces)
        socketio = current_app.config['SOCKETIO']
        recent_potholes = []
        for det in detections:
            recent_potholes.append({
                'lat': lat_val or 17.6868,
                'lon': lon_val or 83.2185,
                'severity': det["Severity"]
            })
    ...
    # Collect all newly detected potholes
    recent_potholes = []
    for det in detections:  # Reuse detections list
        recent_potholes.append({
            'lat': lat_val,
            'lon': lon_val,
            'severity': det["Severity"],
            'area_cm2': det["Area_cm2"],
            'message': f'Severity: {det["Severity"]}, Area: {det["Area_cm2"]}cm²'
        })
    
    # Update global potholes and BROADCAST
    app.config['potholes'] = list(mongo.db.potholes.aggregate([
        {'$lookup': {
            'from': 'pothole_versions',
            'localField': 'current_version',
            'foreignField': '_id',
            'as': 'version_info'
        }},
        {'$unwind': '$version_info'},
        {'$project': {
            '_id': 0,
            'lat': '$version_info.image_id',  # You'll need to join with images
            'lon': '$version_info.image_id',
    ...
    socketio.emit('new_pothole', {
        'potholes': recent_potholes,
        'count': len(detections),
        'message': f'{len(detections)} new potholes detected near {lat_val:.4f}, {lon_val:.4f}!'
    }, broadcast=True)
        # ===== END NEW BLOCK =====
